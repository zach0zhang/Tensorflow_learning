# -*- coding: utf-8 -*-
"""
Created on Wed Nov  7 11:04:58 2018
...
#在这个程序中将用到dropou  方法，dropout可以进一步提升模型可靠性并防止过拟合
#dropout过程只在训练时使用
def inference(input_tensor, train, regularizer):
    #声明第一层卷积层的变量并实现前向传播过程
    #通过使用不同的命名空间来隔离不同层的变量，这可以让每一层中的变量命名只需要考虑当前层的作用，而不需要担心重命名的问题
    ...
    #这一层的输入是上一层的输出，也就28x28x32的矩阵。输出为14x14x32的矩阵
    ...
    with tf.variable_scope('layer3-conv2'):
        ...
        #使用边长为5，深度为64的过滤器，过滤器移动的步长为1，且使用用0填充
    ...
    #第四层的输出为7x7x64d的矩阵，然而第五层全连接层需要的输入格式为向量，所以在这里需要将这个7x7x64的矩阵拉直成一个向量
